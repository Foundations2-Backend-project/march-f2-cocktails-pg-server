...
import { formatCocktails } from './munge-utils.js';
...
app.get('/api/me/favorites', async (req, res) => {
  ...
  try {
    ...
    SELECT id, drink_id, name, category, alcohol_present as "alcoholPresent", glass, instructions, image, ingredient1, ingredient2, ingredient3, ingredient4, ingredient5, ingredient6, ingredient7, ingredient8, ingredient9, ingredient10, ingredient11, ingredient12, ingredient13, ingredient14, ingredient15
    FROM favorites
    WHERE user_id = $1;
    `, [req.userId]);
...
app.get('/api/cocktails/:id', async (req, res) => {
  try{
    ...
    const response = await request.get(`https://www.thecocktaildb.com/api/json/v2/${process.env.COCKTAIL_API_KEY}/lookup.php?i=${req.params.id}`);
    // .query({ api_key: process.env.COCKTAIL_API_KEY });
