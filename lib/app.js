...
//               ingredient7, ingredient8, ingredient9, ingredient10, ingredient11, ingredient12, ingredient13, ingredient14, ingredient15, user_id as "userId", u.name as "userName"
//       FROM    favorites f
//       JOIN    users u
//       ON      f.user_id = u.id
//       WHERE   drink_id = $1;
...
app.get('/api/cocktails', async (req, res) => {
  ...
  try {
    const response = await request.get('www.thecocktaildb.com/api/json/v1/1/search.php')
      .query({ api_key: process.env.COCKTAIL_API_KEY })
      .query({ s: req.query.search });
...
app.get('/api/cocktails/:id', async (req, res) => {
  try{
    ...
    const response = await request.get(`www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${req.params.id}`)
      .query({ api_key: process.env.COCKTAIL_API_KEY });
   
    const cocktails = formatCocktailDetail(response.body.drinks[0]);
